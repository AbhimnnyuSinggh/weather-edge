...
def _format_lockin(a: AlertReady, balance: float) -> str:
    ...
    return (
        ...
        f"🌡️ METAR confirms:\n"
        f"{a.metar_summary}\n"
        f"High set probability: {a.win_probability*100:.0f}%\n\n"
        f"✅ BUY YES \"{a.bin_label}\" at {a.entry_price*100:.0f}¢\n"
        f"   Edge: +{int(max(0, a.win_probability - a.entry_price) * 100)}% | Shares: {a.sized_shares:.0f} | Cost: ${a.sized_cost:.2f}\n"
        f"   Win prob: {a.win_probability*100:.0f}% | Profit if win: ${a.sized_profit_if_win:.2f} | EV: +${a.sized_ev:.2f}\n"
        f"   Market: {a.polymarket_url}\n"
        f"   {kelly_str}\n\n"
        f"💰 Balance: ${balance:.2f} | After trade: ${a.available_after:.2f}"
    )
...
def _format_no_tail(a: AlertReady, balance: float) -> str:
    ...
    return (
        ...
        f"{update_str}"
        f"Score: {a.confidence_score}/100 | {_ist_time()}\n\n"
        f"🌡️ Day high is SET and falling\n"
        f"{a.metar_summary}\n"
        f"Decay impossibility: {a.win_probability*100:.0f}%\n\n"
        f"⛔ BUY NO \"{a.bin_label}\" at {a.entry_price*100:.0f}¢ (YES at {yes_price*100:.0f}¢)\n"
        f"   Edge: +{int(max(0, a.win_probability - a.entry_price) * 100)}% | Shares: {a.sized_shares:.0f} | Cost: ${a.sized_cost:.2f}\n"
        f"   Win prob: {a.win_probability*100:.0f}% | Profit if win: ${a.sized_profit_if_win:.2f} | EV: +${a.sized_ev:.2f}\n"
        f"   Market: {a.polymarket_url}\n"
        f"   {kelly_str}\n\n"
...
def _format_ladder(a: AlertReady, balance: float) -> str:
    ...
    return (
        ...
        f"🌡️ Models disagree:\n"
        f"{a.model_summary}\n\n"
        f"━━ BUY YES on ALL {len(a.bins)} BINS ━━\n"
        f"{bin_lines}\n"
